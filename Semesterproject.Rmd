...
The data for this project was collected in the context of the [Evalutation eiens akustischen Wildschwein-Vergrämungssystems](https://www.zhaw.ch/de/forschung/forschungsdatenbank/projektdetail/projektid/842/). Further information about this study can be found in the [final report](https://www.zhaw.ch/storage/hochschule/medien/bildmaterial/Schlussbericht_Wildschweinschreck.pdf) (language: German) of the study and the [R package](https://computationalmovementanalysis.github.io/ComputationalMovementAnalysisData/index.html) of the data. 
...
# colors
col_6<-c("#762a83", "#af8dc3", "#e7d4e8", "#d9f0d3", "#7fbf7b", "#1b7837")
```
```{r, warning=FALSE}
...
rm( f_join_sector, m_join_sector, f_a, m_a)
```
```{r }
...
Bounding box of all points of f/m_encounters
   xmin    ymin    xmax    ymax 
2568440 1202900 2574921 1207596 
...
st_crs(bb_desc)
f_boden<-st_join(f_encounters, bb_desc)
m_boden<-st_join(m_encounters, bb_desc)

ggplot()+
  geom_sf(data=m_sector)
  geom_sf(data=m_boden)+
  geom_path(data=m_boden, aes(E,N), color="grey", alpha=.5)+
  theme_minimal()+
  coord_sf(datum=2056)
...
ggplot(data=comparison, aes(sex, speed, fill=sex))+
  ...
  ylab("speed [m/s]")+
  scale_fill_manual(values=c(col_6[2], col_6[5]))
...
f_speed_modus<- f_1h %>%
  ...
  ylab("speed [m/s]")+
  scale_fill_manual(values=c(col_6[1], col_6[2], col_6[4], col_6[5]))+
  theme_minimal()
...
m_speed_modus<- m_1h %>%
  ...
  geom_boxplot(aes(x=modus, y=speed, group=modus, fill=modus)) +
  ylab("speed [m/s]")+
  scale_fill_manual(values=c(col_6[2], col_6[4], col_6[5]))+
  theme_minimal()
...
f_speed_vol<- f_1h %>%
  ...
  ylab("speed [m/s]")+
  scale_fill_manual(values=c(col_6[5], col_6[4], col_6[2], col_6[1]))+
  theme_minimal()
...
m_speed_vol<- m_1h %>%
  ...
  geom_boxplot(aes(x=lautstaerke, y=speed, group=lautstaerke, fill=as.factor(lautstaerke)))+
  ylab("speed [m/s]")+
  scale_fill_manual(values=c( col_6[5], col_6[4]))+
  theme_minimal()
...
f_dist_encounter<- f_dist %>%
  ...
  scale_fill_manual(values=col_6)+
  theme_minimal()
