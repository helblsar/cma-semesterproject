...
f_1h_encounters_csv <- f_1h_encounters %>%
  st_drop_geometry() %>%
  write.csv("C:/Users/sarah/OneDrive - ZHAW/ACLS/Track_Module/project/Project-template-master/Project-template-master/f_1h_encounters.csv", row.names = FALSE)
m_1h_encounters_csv <- m_1h_encounters %>%
  st_drop_geometry() %>%
  write.csv("C:/Users/sarah/OneDrive - ZHAW/ACLS/Track_Module/project/Project-template-master/Project-template-master/m_1h_encounters.csv", row.names = FALSE)

```
```{r }
# how about plotting out all encounters...
encounter_plot <- function (in_data, nr) {
  encounter_nr <- in_data %>%
    group_by(meet_id, encounter) %>%
    filter(meet_id == nr)
  bufferid<-(encounter_nr$id[1])
  buffer<- filter(det_buffer, id==bufferid)
  nr_plot <-ggplot()+
    geom_sf(data=buffer, fill="grey", alpha=0.7)+
    geom_sf(data=encounter_nr, aes(colour=encounter), shape=2)+
    geom_path(data=encounter_nr, aes(E, N), color="grey")+
    coord_sf(datum=2056)
  return(nr_plot)
}
...
# let's see the first 5 encounters of each female and male wild boars
#f_plot1<- encounter_plot(f_1h_encounters, 1)
#f_plot2<- encounter_plot(f_1h_encounters, 2)
#f_plot3<- encounter_plot(f_1h_encounters, 3)
#f_plot4<- encounter_plot(f_1h_encounters, 4)
#f_plot5<- encounter_plot(f_1h_encounters, 5)

#m_plot1<- encounter_plot(m_1h_encounters, 1)
#m_plot2<- encounter_plot(m_1h_encounters, 2)
#m_plot3<- encounter_plot(m_1h_encounters, 3)
#m_plot4<- encounter_plot(m_1h_encounters, 4)
#m_plot5<- encounter_plot(m_1h_encounters, 5)
...
# speed within the 12h around the encounter by year
# MAYBE THIS WHOLE IDEA IS SHIT
# MAYBE YEAR SHOULD BE SPLIT UP INTO MONTHS (THE SPIKES ARE EXTREM) -> and maybe the entire idea for such a plot is idiotic 


f_speed_14<-f_encounters %>%
  filter(year(DatetimeRound)==2014)%>%
  ggplot(aes(x=DatetimeRound, y=speed, color=meet_id))+
  geom_line()
f_speed_15<-f_encounters %>%
  filter(year(DatetimeRound)==2015)%>%
  ggplot(aes(x=DatetimeRound, y=speed, color=meet_id))+
  geom_line()
f_speed_16<-f_encounters %>%
  filter(year(DatetimeRound)==2016)%>%
  ggplot(aes(x=DatetimeRound, y=speed, color=meet_id))+
  geom_line()


m_speed_14<-m_encounters %>%
  filter(year(DatetimeRound)==2014)%>%
  ggplot(aes(x=DatetimeRound, y=speed, color=meet_id))+
  geom_line()
m_speed_15<-m_encounters %>%
  filter(year(DatetimeRound)==2015)%>%
  ggplot(aes(x=DatetimeRound, y=speed, color=meet_id))+
  geom_line()
m_speed_16<-m_encounters %>%
  filter(year(DatetimeRound)==2016)%>%
  ggplot(aes(x=DatetimeRound, y=speed, color=meet_id))+
  geom_line()
```
